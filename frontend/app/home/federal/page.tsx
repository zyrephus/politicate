  ...
  useEffect(() => {
    const fetchNews = async () => {
      try {
        ...
        // Fetch news using the postal code
        const response = await fetch(
          `http://localhost:8000/getNews/Federal/${postalData.postalCode}`
        );
        ...
        const transformedNews = data.map((item: NewsItem) => ({
          title: item.title,
          link: item.link,
          snippet: item.response, // Using the AI-generated summary as the snippet
  ...
  return (
    <div className="relative min-h-screen">
      <div className="absolute inset-0 w-full z-10">
        ...
        >
          <div className="mb-8">
            <h1 className="text-3xl font-bold">Federal News</h1>
            <p className="text-gray-500 mt-2">
              Stay informed about Canadian federal politics and national
              decisions.
            </p>
          </div>
